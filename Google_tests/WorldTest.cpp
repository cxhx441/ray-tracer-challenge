...
TEST(WorldTestSuite, CreateDefaultWorld){
    ...
    ASSERT_EQ(w.shapes.size(), 2);

    EXPECT_EQ(w.shapes[0]->material.color, Color(0.8, 1.0, 0.6, 1) );
    EXPECT_FLOAT_EQ(w.shapes[0]->material.diffuse, 0.7);
    EXPECT_FLOAT_EQ(w.shapes[0]->material.specular, 0.2);

    EXPECT_EQ(w.shapes[1]->get_transform(), Transformation::scaling(0.5) );
}
...
TEST(WorldTestSuite, RefractedColorForOpaqueMaterialIsBlack) {
    World w = World::DefaultWorld();
    std::shared_ptr<Shape> shape = w.shapes[0];
    Ray r = Ray(Tuple::point(0, 0, -5), Tuple::vector(0, 0, 1));
...
//    Sphere &A = w.spheres[0];
    std::shared_ptr<Shape> A = w.shapes[0];
    A->material.ambient = 1;
    A->material.set_pattern(TestPattern());
    A->name = "A";

    std::shared_ptr<Shape> B = w.shapes[1];
...
//    EXPECT_EQ(w.shapes[0]->material.refractive_index, 1);
//    EXPECT_EQ(w.shapes[1]->material.refractive_index, 1.5);
//}
