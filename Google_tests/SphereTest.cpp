...
TEST(SphereTestSuite, NormalOnSphereAtNonAxialPoint) {
    Sphere s = Sphere();
    Tuple n = s.NormalAt(Tuple::point(sqrt(3) / 3.f, sqrt(3)/ 3.f, sqrt(3) / 3.f));
...
TEST(SphereTestSuite, NormalIsNormalizedVector) {
    Sphere s = Sphere();
    Tuple n = s.NormalAt(Tuple::point(sqrt(3) / 3.f, sqrt(3)/ 3.f, sqrt(3) / 3.f));
    EXPECT_EQ(n, Tuple::normalize(Tuple::vector( sqrt(3) / 3.f, sqrt(3)/ 3.f, sqrt(3) / 3.f)));
}

TEST(SphereTestSuite, ComputeNormalOnTranslatedSphere) {
    Sphere s = Sphere();
    s.transformation = Transformation::translation(0, 1, 0);
